...
namespace Dialog.Syntaxes
{
    ...
    public struct Range : IComparable<Range>
    {
        ...
        public static Range Parse(string s)
        {
            ...
            if (split.Length == 1)
            {
                double v = ValueParser.ToDouble(split[0]) ?? ThrowInvalidNum();
                return From(v).To(v);
            }
            ...
            {
                return UpTo(ValueParser.ToDouble(split[1]) ?? ThrowInvalidNum());
            }

            //Open end (n..)
            else if (string.IsNullOrEmpty(split[1]))
            {
                return From(ValueParser.ToDouble(split[0]) ?? ThrowInvalidNum());
            }
            ...
            {
                return From(ValueParser.ToDouble(split[0]) ?? ThrowInvalidNum())
                    .To(ValueParser.ToDouble(split[1]) ?? ThrowInvalidNum());
            }

            /* local */ double ThrowInvalidNum()
            {
                throw new InvalidCastException("Invalid number in range specifier");
            }
