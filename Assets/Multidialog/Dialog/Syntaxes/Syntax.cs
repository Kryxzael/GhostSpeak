...
namespace Dialog.Syntaxes
{
    ...
    public class Syntax
    {
        ...
        public static readonly Syntax Empty = new Syntax();
        ...
        /// </summary>
        public int ArgumentCount => Items.Count;

        /// <summary>
        /// Returns true if this syntax has an alternate syntax
        /// </summary>
        public bool HasAlternateSyntax => AlternateSyntax != null;
        ...
        /// <returns></returns>
        internal bool IsCorrectSyntax(Params args)
        {
            if (args.Count < Items.Count)
            {
                return (AlternateSyntax?.IsCorrectSyntax(args)).GetValueOrDefault(false);
            }
            ...
            for (int i = 0; i < Items.Count; i++)
            {
                switch (Items[i].Type)
                {
                    ...
                    case SyntaxItemType.Number:
                        if (args.IsDouble(i) && Items[i].Range.IsInRange(args.ToFloat(i)))
                        {
                            continue;
                        }
